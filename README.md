...
| `ALIYUNCS_API_KEY`    | 否   | -                           | 阿里云 API Key          |
| `PRIMARY_MODEL`       | 否   | aliyuncs/qwen3.5-plus      | 主用模型                |
| `PRIMARY_VL_MODEL`    | 否   | aliyuncs/qwen3-vl-plus     | 视觉模型                |
...
| `aliyuncs/qwen3-vl-plus` | 文本+视觉 | 阿里云 Qwen3-VL-Plus (默认视觉模型) |
...
配置文件 `config/openclaw.json` 采用模板形式，挂载到容器后会自动读取环境变量。主要配置项包括：

- **模型配置** - providers (minimax, aliyuncs)
- **渠道配置** - channels (feishu, wecom)
- **网关配置** - gateway (端口、认证、速率限制)
- **浏览器配置** - browser (无头模式、无沙箱)
...
| `./prod/workspace` | `/root/.openclaw/workspace` | 工作空间       |
| `./prod/agents`   | `/root/.openclaw/agents`    | Agent 配置     |
| `./prod/canvas`   | `/root/.openclaw/canvas`   | Canvas 数据    |
| `./prod/credentials` | `/root/.openclaw/credentials` | 凭证信息     |
| `./prod/cron`     | `/root/.openclaw/cron`      | 定时任务       |
| `./prod/logs`     | `/root/.openclaw/logs`      | 日志文件       |
| `./prod/memory`   | `/root/.openclaw/memory`    | 记忆数据       |
...
- `prod/workspace/` - 对话历史、文件等
- `prod/agents/` - Agent 配置
